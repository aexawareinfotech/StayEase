...
const AdminReports = () => {
  ...
  const exportToCSV = () => {
    ...
    report.bookings.forEach((b) => {
      csvRows.push(
        `${b.bookingId || b._id},${b.status},${b.totalPrice || b.totalAmount},${b.checkIn.split("T")[0]},${b.checkOut.split("T")[0]}`,
      );
    });
      ...
      <div className="card border-0 shadow-sm rounded-4 mb-5 p-4 bg-white">
        <form onSubmit={handleGenerate} className="row g-3 align-items-end">
          <div className="col-md-4">
            <label className="form-label text-muted fw-bold small text-uppercase tracking-wider">
              Start Date
            </label>
            <input
              type="date"
          ...
          <div className="col-md-4">
            <button
              type="submit"
              disabled={loading}
              className="btn btn-primary btn-lg w-100 fw-bold rounded-pill shadow transition hover-scale"
              style={{
        ...
        <div className="col-md-4">
          <div className="card shadow-sm border-0 rounded-4 h-100 bg-primary bg-opacity-10 text-primary hover-scale transition">
            <div className="card-body p-4 d-flex flex-column align-items-start justify-content-center">
              <h6 className="fw-bold tracking-wider text-uppercase small mb-2 opacity-75">
                Generated Revenue
              </h6>
              <h2 className="fw-bolder mb-0">
                ₹ {report?.totalRevenue?.toLocaleString("en-IN") || 0}
              </h2>
            </div>
          </div>
        </div>
